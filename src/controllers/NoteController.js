...
export const insertNote = (data) => {
  ...
  return user_id && body && title ? pushNote(user_id, body, title) : null;
};
...
export const updateNoteTitle = (data) => {
  ...
  return user_id && note_id && title
    ? updNoteTitle(user_id, note_id, title)
    : null;
};
...
export const updateNoteBody = (data) => {
  ...
  return user_id && note_id && body
    ? updNoteBody(user_id, note_id, body)
    : null;
};
...
export const getNote = (data) => {
  ...
  return user_id && note_id ? gNote(user_id, note_id) : null;
};

export const getNotes = (data) => {
  const { user_id } = data;

  return user_id ? gNotes(user_id) : null;
};
